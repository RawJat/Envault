...
### 5. Headless Execution Paradigms

Envault now has two distinct headless auth models:

- **CI/CD integrations:** read-only Service Tokens (`envault_svc_`) used with `envault run` for runtime injection.
- **AI agents (MCP):** short-lived delegated JWTs (`envault_agt_`) issued by `envault mcp install`, with HITL approval on mutations.

This split prevents CI automation from mutating secrets while still enabling scoped agent workflows with explicit human approval.
