...
| `VERCEL_REDIRECT_URI`            | Exact callback URI registered in Vercel integration settings                  | For Vercel integration                     |
| `VERCEL_WEBHOOK_SECRET`          | Vercel webhook signing secret for installation removal events                 | Strongly recommended for Vercel integration |
| `NEXT_PUBLIC_VERCEL_INTEGRATION_INSTALL_URL` | Public install URL for your Vercel integration listing             | For Vercel integration                     |
...
### Vercel Native Integration Post-Deploy

After deployment, complete these one-time steps for native sync:

1. In Vercel integration settings, set callback URL to:
  `https://<your-domain>/api/integrations/vercel/callback`
2. Set webhook URL to:
  `https://<your-domain>/api/integrations/vercel/webhook`
3. Install integration from your production app/integration URL.
4. In Envault project integration UI, link Vercel project, save environment mapping, and run initial sync.
