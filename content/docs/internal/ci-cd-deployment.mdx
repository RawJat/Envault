...
<Callout type="warn">
  For Vercel serverless runtime variables, prefer Envault native Vercel integration.
  `envault run` remains correct for build/test/start commands in generic CI pipelines,
  but should not be the only mechanism for Vercel runtime env propagation.
</Callout>
