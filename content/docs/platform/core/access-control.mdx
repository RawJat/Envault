...
<Callout type="info">
  ...
  the project, and can also manage existing shares. Owners must approve new
  share requests that grant environment-scoped access.
</Callout>
...
<Callout type="info">
  **GitHub Collaborators bypass the "Pending Approval" flow entirely.** Once
  auto-granted, the user is a full `Viewer` member of the project. They will see
  the project in their "Shared with me" dashboard, can run subsequent CLI
...
See the [GitHub Integration](/docs/platform/guides/github-integration) guide for setup instructions.
