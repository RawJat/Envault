...
- **y** - An access request is sent to the project owner. The owner receives an email and an in-app notification, including the requested environment when `--env` is used. Once approved, the developer can run `envault pull` again.
