...
## SDK and Agent Security

Envault SDK and MCP mutation workflows are protected by multiple controls:

- **Delegated agent tokens**: machine tokens are issued as scoped `envault_agt_` JWTs.
- **Context fencing**: delegated tokens are validated against project scope before mutation workflows continue.
- **Rate limiting**: SDK auth path includes per-agent rate limiting.
- **Kill switches**: both global user-level and project-level agent access must be enabled.
- **Human approval**: mutation execution requires explicit owner/editor approval.
- **Approval replay protection**: approved payload retrieval is one-time consumable.
- **Audit coverage**: approval outcomes are captured as dedicated audit actions.
