...
export function ShareProjectDialog({
  ...
  const handleMemberRoleChange = (
    ...
    if (newValue === member.role) {
      if (
        ...
        !areEnvironmentsEqual(
          existingChange.allowedEnvironments,
  ...
  const handleModifyEnvironments = (
    ...
    const isRoleSame =
      !existingChange ||
      (existingChange.type === "role_change" &&
        existingChange.newRole === member.role);
  ...
  const handleSave = () => {
    ...
    const invalidEnvChanges =
      project.ui_mode === "advanced" &&
      project.environments &&
      project.environments.length > 0
        ? Array.from(pendingChanges.values()).filter((c) => {
            ...
            if (c.type === "approve" || c.type === "role_change") {
              ...
              if (
                c.allowedEnvironments !== undefined &&
                c.allowedEnvironments.length === 0
              )
                return true;
            }
            return false;
    ...
    if (allInvalidChanges.length > 0) {
      const firstInvalidId =
        allInvalidChanges[0].requestId || allInvalidChanges[0].userId; // active members use userId for the accordion ID
      ...
      toast.error(
        "Please explicitly assign a role and at least one environment to all active members.",
      );
      return;
    }
        ...
        <DialogContent
          ...
          onInteractOutside={(e) => {
            if (hasChanges) {
              e.preventDefault();
              toast.error(
                "You have unsaved changes. Discard changes to proceed to the dashboard.",
              );
              setModalShake(false);
              setTimeout(() => {
                setModalShake(true);
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="grid gap-2">
                          {pendingRequests.map((request) => {
                            ...
                            const currentAction = getCurrentValue(
                              request.user_id,
                              "pending",
                            );
                            const existingChange = pendingChanges.get(
                              request.user_id,
                            );
                            ...
                            return (
                              <div
                                key={request.id}
                                className={`flex flex-col border rounded-lg overflow-hidden transition-all bg-muted/30 ${shakeIds.has(request.user_id) ? "animate-shake border-destructive/50 ring-1 ring-destructive/50" : ""}`}
                              >
                                <div
                                  className={`flex flex-row items-center p-3 gap-3 w-full ${currentAction === "approve" ? "cursor-pointer hover:bg-muted/50" : ""}`}
                                  onClick={(e) => {
                                    if (currentAction === "approve")
                                      toggleExpand(e, request.id);
                                  }}
                                >
                                  <UserAvatar
                                  ...
                                  <div className="flex items-center gap-2 flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center space-x-2 shrink-0">
                                      {isOwner && (
                                        ...
                                        >
                                          <SelectTrigger
                                            className="w-[100px] h-8 text-xs sm:text-sm sm:h-9 sm:w-32"
                                          ...
                                          project.environments.length > 0 && (
                                            <div className="flex flex-col gap-2">
                                              ...
                                              <div className="flex flex-row flex-wrap gap-4 items-center rounded-md border border-input bg-background px-3 py-3 text-sm">
                                                {getAllProjectEnvSlugs().map(
                                                  (env) => {
                                                    const isChecked =
                                                      currentEnvs.includes(env);
                                                    return (
                                                      <div
                                                        key={`pending-${request.id}-${env}`}
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="grid gap-2">
                          {members.map((member) => {
                            ...
                            const currentRole = getCurrentValue(
                              member.user_id,
                              member.role,
                            );
                            const currentEnvs = getCurrentEnvironments(
                              member.user_id,
                              member.allowed_environments,
                            );
                            ...
                            return (
                              <div
                                key={member.id}
                                className={`flex flex-col border rounded-lg overflow-hidden transition-all bg-card ${shakeIds.has(member.user_id) ? "animate-shake border-destructive/50 ring-1 ring-destructive/50" : ""}`}
                              >
                                <div
                                  className={`flex flex-row items-center p-3 gap-3 w-full ${!isOwnerRole && isOwner ? "cursor-pointer hover:bg-muted/50" : ""}`}
                                  onClick={(e) => {
                                    if (!isOwnerRole && isOwner)
                                      toggleExpand(e, member.id);
                                  }}
                                >
                                  <UserAvatar
                                    className="h-8 w-8 shrink-0"
