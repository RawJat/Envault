...
export function ManualInstallSelector() {
    ...
    useEffect(() => {
        if (!ARCH_MAP[os].includes(arch)) {
            setArch(ARCH_MAP[os][0])
        }
    ...
    const completedCount = Array.from(completedSteps).filter(step => step <= interactiveSteps).length
        ...
        <div className="my-8 border border-muted rounded-xl overflow-hidden bg-card/50 backdrop-blur-sm shadow-sm">
            <div className="p-4 border-b border-muted bg-muted/30 flex flex-row items-center gap-6">
                <div className="space-y-1">
                    <label className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold">Operating System</label>
                ...
                <div className="space-y-1">
                    <label className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold">Architecture</label>
                    <Select value={arch} onValueChange={(v) => setArch(v as Arch)}>
                        ...
                        <SelectContent>
                            {ARCH_MAP[os].map((a) => {
                                let label = ''
                                if (os === 'Windows') {
                                    label = a === 'x86_64' ? 'x86-64 (Intel/AMD)' : 'ARM64 (Qualcomm)'
                                } else if (os === 'macOS') {
                                    label = a === 'arm64' ? 'ARM64 (Apple Silicon)' : 'x86-64 (Intel)'
                                } else if (os === 'Linux') {
                                    label = a === 'x86_64' ? 'x86/x86-64' : 'ARM/ARM64'
                                }
                                return (
                                    <SelectItem key={a} value={a} className="text-xs">{label}</SelectItem>
                                )
                            })}
                        </SelectContent>
                    </Select>
            ...
            <div className="p-6">
                <Steps>
                    <Step>
                        ...
                        <Button asChild className="h-10" variant="outline" onClick={handleDownload}>
                            <a href={downloadUrl} target="_blank" rel="noreferrer">
                                <Download className="w-4 h-4" />
                                {binaryName}
                            </a>
                        </Button>
                        <p className="mt-2 text-xs text-muted-foreground">Or copy the link to use with <code>wget</code> or <code>curl</code>.</p>
                    ...
                    <Step>
                        <h4 className="font-bold mb-3">{os === 'Windows' ? 'Add to PATH' : 'Move to PATH'}</h4>
                        {os === 'Windows' ? (
                            <>
                                <p className="text-sm text-muted-foreground leading-relaxed mb-3">
                                    Add the directory containing <code>envault.exe</code> to your User or System Environment Variables to use it from anywhere.
                                </p>
                                <div className="bg-muted/50 rounded-lg p-4 text-sm space-y-2">
                                    <p className="font-semibold">Steps to add to PATH:</p>
                                    <ol className="list-decimal list-inside space-y-1 text-muted-foreground">
                                        <li >Open System Properties
                                        ...
                                        <li>Under User Variables, select <code className="font-mono">Path</code> and click Edit</li>
                                        <li>Click New and add the folder path containing envault.exe</li>
                            ...
                            <>
                                ...
                                <p className="mt-3 text-sm text-muted-foreground leading-relaxed">
                                    Moving the binary to <code>/usr/local/bin</code> makes it available system-wide as the <code>envault</code> command.
                                </p>
                            </>
                        )}
                    </Step>
                ...
                <div className="pt-4 mt-4 border-t border-muted flex items-center gap-3">
                    {allStepsComplete ? (
                        ...
                        <div className="flex items-center gap-2 px-3 py-1 bg-blue-500/10 text-blue-600 rounded-full text-[10px] font-bold uppercase tracking-wider">
                            <span>{completedCount}/{interactiveSteps} Steps</span>
                        </div>
                    )}
                    <span className="text-xs text-muted-foreground font-mono">envault --version</span>
                </div>
            </div>
        </div>
    )
