...
import { ShieldCheck, Settings as SettingsIcon, LogOut, Search, Keyboard, Activity, ArrowLeft } from "lucide-react"
...
export function AppHeader({ title, backTo, actions, hideSearch = false }: AppHeaderProps) {
        ...
        <header className="border-b sticky top-0 bg-background/95 backdrop-blur z-50">
            <div className="container mx-auto py-4 px-4 flex items-center justify-between">
                <div className="flex items-center gap-2">
                    {backTo ? (
                        <Button variant="ghost" size="icon" onClick={() => router.push(backTo)} className="[&_svg]:!w-5 [&_svg]:!h-5">
                            <ArrowLeft />
                        </Button>
                    ) : (
                        !title && (
                            ...
                            {(user?.app_metadata?.is_admin === true || user?.user_metadata?.is_admin === true) && (
                                <>
                                    <DropdownMenuItem asChild>
                                        <Link href="/admin/status" className="cursor-pointer flex w-full items-center">
