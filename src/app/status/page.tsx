...
export default async function StatusPage() {
    ...
    const pastIncidents = allIncidents.filter((i: any) => i.status === "resolved");
    ...
    const hasMaintenance = components.some((c: any) => c.status === "maintenance");
    ...
    if (hasOutage) {
        ...
        StatusIcon = XCircle;
        statusMessage = "We are currently experiencing a major outage. Our team is investigating.";
            ...
            <main className="flex-1 relative pt-24 md:pt-32 pb-16 md:pb-24 px-4 md:px-6">
                ...
                <div className="max-w-4xl mx-auto space-y-16 relative z-10">
                    ...
                    <div className="text-center space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                        <div className={cn("inline-flex items-center gap-2 md:gap-3 px-4 md:px-6 py-2.5 md:py-3 rounded-full border backdrop-blur-sm", statusBg)}>
                            <StatusIcon className={cn("w-5 h-5 md:w-6 md:h-6 animate-pulse", statusColor)} />
                            <span className={cn("text-base md:text-lg font-bold tracking-tight", statusColor)}>{statusText}.</span>
                        </div>
                    ...
                    <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-200">
                        <div className="flex items-center gap-2 md:gap-3">
                            <h2 className="text-xl md:text-2xl font-serif whitespace-nowrap">System Components</h2>
                            <div className="h-px bg-border flex-1" />
                        </div>
                                ...
                                >
                                    <div className="flex items-center gap-2 md:gap-3 min-w-0 flex-1">
                                        <div className={cn(
                                            ...
                                            component.status === 'operational' ? "bg-emerald-500 ring-emerald-500/20" :
                                                component.status === 'degraded' ? "bg-amber-500 ring-amber-500/20" :
                                                    component.status === 'outage' ? "bg-red-500 ring-red-500/20" :
                                                        "bg-blue-500 ring-blue-500/20"
                                        )} />
                                        <span className="font-medium group-hover:translate-x-1 transition-transform text-sm md:text-base truncate">{component.name}</span>
                                    </div>

                                    <div className="flex items-center gap-1.5 md:gap-2 text-xs md:text-sm text-muted-foreground shrink-0">
                                        <span className="capitalize inline">{component.status.replace('-', ' ')}</span>
                                        {component.status === 'operational' && <CheckCircle2 className="h-3.5 w-3.5 md:h-4 md:w-4 text-emerald-500 opacity-0 group-hover:opacity-100 transition-opacity" />}
                                        {component.status === 'degraded' && <AlertTriangle className="h-3.5 w-3.5 md:h-4 md:w-4 text-amber-500" />}
                                        {component.status === 'outage' && <XCircle className="h-3.5 w-3.5 md:h-4 md:w-4 text-red-500" />}
                                        {component.status === 'maintenance' && <Wrench className="h-3.5 w-3.5 md:h-4 md:w-4 text-blue-500" />}
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-300 mb-6">
                        <div className="flex items-center gap-2 md:gap-3">
                            <h2 className="text-xl md:text-2xl font-serif whitespace-nowrap">Past Incidents</h2>
                            <div className="h-px bg-border flex-1" />
                        </div>
                        ...
                        {pastIncidents.length === 0 ? (
                            <div className="p-6 md:p-8 text-center rounded-lg border border-dashed bg-muted/20">
                                ...
                                <p className="text-sm md:text-base text-muted-foreground">No incidents reported in the past 90 days.</p>
                            </div>
                        ) : (
                            <Accordion type="single" collapsible className="w-full space-y-4">
                                {pastIncidents.map((incident: any) => (
                                    <AccordionItem key={incident.id} value={incident.id} className="border rounded-lg bg-card/20 px-3 md:px-4">
                                            ...
                                            <div className="flex flex-wrap sm:flex-nowrap items-center gap-2 sm:gap-3 text-left w-full">
                                                ...
                                                <div className="flex items-center gap-2 w-full sm:w-auto">
                                                    <div className={cn(
                                                        ...
                                                        incident.severity === 'minor' ? 'bg-blue-500' : 'bg-orange-500'
                                                    )} />
                                                ...
                                                <div className="flex items-center gap-2 flex-1 min-w-0 w-full sm:w-auto pl-3.5 sm:pl-0">
                                                    <span className="font-medium flex-1 text-sm md:text-base break-words min-w-0">{incident.title}</span>
                                                    <Badge variant="outline" className="mr-2 font-normal text-xs bg-emerald-500/10 text-emerald-500 border-emerald-500/20 shrink-0">{incident.status.charAt(0).toUpperCase() + incident.status.slice(1)}</Badge>
                                                </div>
                                            </div>
                                        </AccordionTrigger>
