...
export async function shareSecret(secretId: string, email: string) {
    ...
    const project = (Array.isArray(secret.project) ? secret.project[0] : secret.project) as { name: string }
    ...
    if (insertError) {
        if (insertError.code === '23505') return { error: 'User already has access' } // Unique constraint
