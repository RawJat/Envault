...
export default async function JoinPage({ params }: JoinPageProps) {
  ...
  if (isFullMember && projectData) {
    // Already has full access, redirect to project page
    // Fetch the owner's username separately to build the correct URL
    const { data: ownerProfile } = await adminSupabase
      .from("profiles")
      .select("username")
      .eq("id", projectData.user_id)
      .maybeSingle();
    const ownerUsername = ownerProfile?.username || "user";
    const slug = projectData.slug;

    if (projectData.user_id === user.id) {
      redirect(`/project/${slug}`); // Owned projects
    } else {
      redirect(`/${ownerUsername}/${slug}`); // Shared projects
    }
  }
        ...
        <div className="w-[90vw] sm:w-full sm:max-w-md mx-auto">
          <Card className="border-muted/40 shadow-2xl backdrop-blur-sm bg-background/80">
            <CardHeader className="text-center space-y-2">
              ...
              <CardDescription>
                The project you are trying to join does not exist or has been
                deleted.
              </CardDescription>
            </CardHeader>
            <CardContent>
